...
echo "REACT_APP_API_ENDPOINT=$API_ENDPOINT" > $PROJECT_ROOT/packages/frontend/.env
...
aws s3 sync $PROJECT_ROOT/packages/frontend/build/ s3://$FRONTEND_BUCKET --delete --region $REGION
